...
`op` is either a `DockerOp` string or `set_runtime` /
`set_interval`. `set_runtime` payload is `AgentRuntime` JSON
...
Disconnect marks the node not-connected and fails in-flight calls.
