...
Do not `sudo cargo`. Prefer `TMPDIR=$PWD/.smoke/tmp` if `/tmp` is full
(absolute — a relative path breaks `keystone-proto` because cargo runs that
build script in the crate dir). Smoke
...
not a timezone textbox) is in. **Unattended-upgrades** (`unattended_set`,
KeyStone drop-in `/etc/apt/apt.conf.d/99-keystone-unattended` plus
`systemctl enable`/`disable` without `--now`) is in; editing `20auto-upgrades` is not.
