...
| POST | `/nodes/{id}/sys/{op}` | `{op}` is `SysOp::as_str()`. Form JSON or `iface` / `method` / IPv4 fields. Optional `totp`. `net_set` requires a current authenticator code when TOTP is on (`needs_step_up`); backup codes are rejected; TOTP off is confirm-only. Failed step-up redirects `?panel=system&err=step-up` (or `step-up-locked`) and audits `ok` false. Audit log on mutate. Streaming ops redirect to their follow page (`updates_apply` → apply, `updates_autoremove` → autoremove, `gitlab_backup` → backup, `journal` → System tab). `reboot` is mutating and not streamed. |
