...
sudo cp keystone-server_0.1.0-10_amd64.deb keystone-agent_0.1.0-7_amd64.deb /tmp/

# The one UI box (optional: agent as well, so this host is in the list)
sudo apt install /tmp/keystone-server_0.1.0-10_amd64.deb
sudo apt install /tmp/keystone-agent_0.1.0-7_amd64.deb   # optional on the UI host

# Every other node: agent only
sudo apt install /tmp/keystone-agent_0.1.0-7_amd64.deb
```
...
`apt install --reinstall ./….deb`. A newer revision (`0.1.0-10`) is a
normal upgrade.
