...
sudo cp keystone-server_0.1.0-21_amd64.deb keystone-agent_0.1.0-18_amd64.deb /tmp/

# The one UI box (optional: agent as well, so this host is in the list)
sudo apt install /tmp/keystone-server_0.1.0-21_amd64.deb
sudo apt install /tmp/keystone-agent_0.1.0-18_amd64.deb   # optional on the UI host

# Every other node: agent only
sudo apt install /tmp/keystone-agent_0.1.0-18_amd64.deb
```
...
Current packages enable the unit for boot and start it on configure when
it is enabled (`deb-systemd-invoke start`). `systemctl start` without
`enable` does **not** come back after a reboot. After the UI is up,
**Start KeyStone on boot** on that node's Settings does
...
`apt install --reinstall ./….deb`. A newer revision (`0.1.0-21`) is a
normal upgrade.
