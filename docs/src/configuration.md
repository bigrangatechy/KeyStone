...
| Observe host / Allow apt upgrade, autoremove, IPv4, IPv6, VLAN, leftover restart, GitLab backup, GitLab restore, and reboot | System-admin gates. See [System](system.md). Off until you also enable `keystone-sys.socket` on the node. Settings shows a warning before System Manage: that flag is apt, IPv4, IPv6, VLAN, leftover restart, GitLab backup, GitLab restore, and reboot. |
