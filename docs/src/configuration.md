...
| Observe host / Allow apt upgrade, autoremove, IPv4, GitLab backup, and reboot | System-admin gates. See [System](system.md). Off until you also enable `keystone-sys.socket` on the node. Settings shows a warning before System Manage: that flag is apt, IPv4, GitLab backup, and reboot. |
